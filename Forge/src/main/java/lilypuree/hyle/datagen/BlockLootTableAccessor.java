package lilypuree.hyle.datagen;
...
//    }
    ...
    public static LootTable.Builder createCopperOreDrops(Block $$0) {
        return createSilkTouchDispatchTable($$0, applyExplosionDecay($$0, LootItem.lootTableItem(Items.RAW_COPPER).apply(SetItemCountFunction.setCount(UniformGenerator.between(2.0F, 5.0F))).apply(ApplyBonusCount.addOreBonusCount(Enchantments.BLOCK_FORTUNE))));
    }

    public static LootTable.Builder createLapisOreDrops(Block $$0) {
        return createSilkTouchDispatchTable($$0, applyExplosionDecay($$0, LootItem.lootTableItem(Items.LAPIS_LAZULI).apply(SetItemCountFunction.setCount(UniformGenerator.between(4.0F, 9.0F))).apply(ApplyBonusCount.addOreBonusCount(Enchantments.BLOCK_FORTUNE))));
    }

    public static LootTable.Builder createRedstoneOreDrops(Block $$0) {
        return createSilkTouchDispatchTable($$0, applyExplosionDecay($$0, LootItem.lootTableItem(Items.REDSTONE).apply(SetItemCountFunction.setCount(UniformGenerator.between(4.0F, 5.0F))).apply(ApplyBonusCount.addUniformBonusCount(Enchantments.BLOCK_FORTUNE))));
    }

    public void otherWhenSilkTouch(Block block, ItemLike other) {
